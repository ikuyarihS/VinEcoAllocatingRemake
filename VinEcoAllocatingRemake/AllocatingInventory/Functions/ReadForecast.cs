    ...
    public partial class AllocatingInventory
    {
        ...
        private async void ReadForecast(object sender, DoWorkEventArgs e)
        {
            try
            {
                                        ...
                                        for (var colIndex = 0;
                                            ...
                                            using (DataColumn column = table.Columns[colIndex])
                                            {
                                                ...
                                                dicOldFc.Add(
                                                    ...
                                                    (new SupplierForecast {QualityControlPass = true, SupplierCode = supplierCode, FullOrder = _ulti.ObjectToInt(row["FullOrder"]) == 1, CrossRegion = _ulti.ObjectToInt(row["CrossRegion"]) == 1, LabelVinEco = _ulti.ObjectToInt(row["Label"]) == 1, Level = (byte) _ulti.ObjectToInt(row["Level"])}, false));
                                            }
                                    }
                                }
                            }
                ...
                Parallel.ForEach(
                    new DirectoryInfo($@"{_applicationPath}\Data\Forecast").GetFiles(),
                    new ParallelOptions {MaxDegreeOfParallelism = Environment.ProcessorCount},
                    fileInfo =>
                    {
                    ...
                    table =>
                    {
                        try
                        {
                            ...
                            foreach (DataRow row in table.Select())
                            {
                                ...
                                if (region.Contains(' ')) region = _ulti.ConvertToUnsigned(_ulti.ReturnInitials(region));
                                        ...
                                        dicFc.AddOrUpdate(
                                            ...
                                            (key, oldValue) =>
                                                (new SupplierForecast
                                                {
                                                    ...
                                                    QualityControlPass = true,
                                                    Level =
                                                        (byte) (table.Columns.Contains("Level")
                                                            ? row["Level"] as byte? ?? 1
                                                            : 1),
                                                    QuantityForecast =
                                                        oldValue.Supply.QuantityForecast +
                                                        (isKpi
                                                            ? 0
                                                            : value),
                                                    QuantityForecastPlanned =
                        ...
                        delegate
                        {
                            try
                            {
                                using (var table = new DataTable {TableName = "Forecasts"})
                                {
                                    ...
                                    foreach (DateTime dateFc in listAllDateFc.OrderBy(d => d.Date)) table.Columns.Add(_ulti.DateToString(dateFc, "dd-MMM-yyyy"), typeof(double));
                                        ...
                                        select key)
                                    {
                                        ...
                                        row[_ulti.DateToString(
                                            key.DateFc,
                                            "dd-MMM-yyyy")] = _ulti.DoubleToObject(
